...
def prediction_rank_plot(IOHMM, Base, LSTM, file_name_tail, save_fig):
    ...
    plt.xlabel(r'Prediction rank ($k$)', fontsize=20)
    plt.ylabel('Cumulative probability', fontsize=20)
    plt.title('Middle activities', fontsize=18)
    plt.legend(loc='lower right', fontsize=18)
    ...
    if save_fig == 0:
        ...
        plt.savefig('img/prediction_rank_' + file_name_tail + '.png', dpi=200)
...
if __name__ == '__main__':
    ...
    prediction_rank_plot(IOHMM_cum, base_cum, LSTM_cum, file_name_tail, save_fig = 1)
