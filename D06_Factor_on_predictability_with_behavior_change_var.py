...
def add_zhan_data(data):
    num_change_pts = pd.read_csv('../data/user_change_num_hk.csv')
    assert set(num_change_pts['user_id']) == set(data['Card_ID'])
    data = data.merge(num_change_pts[['user_id','changepoints_freq','changepoints_time','changepoints_loc']], left_on = ['Card_ID'], right_on= ['user_id'])
    return data
...
def run_linear_reg(data_path, data):
    ...
    col_X = ['num_trip_in_day_mean','num_trip_in_day_std','num_days_with_travel','first_departure_time_std',
             'if_student', 'if_senior','if_in_HK_island','if_in_NT','changepoints_time','Total_act']
    col_Y = ['R_sq_dur']
    ...
    table.to_csv('table/estimate_para_on_R_sq_dur_no_entropy_with_zhan_change_var.csv',index=False)
    #############loc
    # col_X = ['num_trip_in_day_mean','num_trip_in_day_std','num_days_with_travel','first_departure_time_std',
    #          'entropy_trip_origin','if_student', 'if_senior']
    col_X = ['num_trip_in_day_mean','num_trip_in_day_std','num_days_with_travel','first_departure_time_std',
             'if_student', 'if_senior','if_in_HK_island','if_in_NT','changepoints_loc','Total_act']
    col_Y = ['acc_loc']
    ...
    table.to_csv('table/estimate_para_on_acc_loc_no_entropy_with_zhan_change_var.csv',index=False)
